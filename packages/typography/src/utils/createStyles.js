...
module.exports = (vr: any, options: OptionsType) => {
  const setAllStyles = (vr, options, masterStyles) => {
    let styles = {}
    ...
    if (masterStyles && isObject(masterStyles)) {
      let trimmedStyles = {}

      each(styles, (declarationBlock, selector, key) => {
        const masterDeclarationBlock = masterStyles[selector]

        each(declarationBlock, (value, property) => {
          let newDeclarationBlock = pickBy(declarationBlock, (value, key) => {
            return value != masterDeclarationBlock[key]
          })

          if (!isEmpty(newDeclarationBlock)) {
  ...
  let styles = setAllStyles(vr, options)

  // Add breakpoints if they're set as an object
  if (options.breakpoints && isObject(options.breakpoints)) {
    each(options.breakpoints, (breakpoint, key) => {
      // new vertical rythm / combined options for breakpoint
      const bpOptions = objectAssign({}, options, breakpoint)
      const bpVr = verticalRhythm(bpOptions)

      // helper function copied from the vr / vertical rythm object in index.js
      bpVr.scale = (value: number) => {
        const baseFont = parseInt(bpOptions.baseFontSize, 10)
        const newFontSize = `${ms(value, bpOptions.scaleRatio) * baseFont}px`

        return bpVr.adjustFontSizeTo(newFontSize)
      }

      let bpStyles = setAllStyles(bpVr, bpOptions, styles)

      styles = set(styles, key, bpStyles)
    })
  }
